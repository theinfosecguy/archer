...
def validate(template_name: str, secret: Optional[str], var_args: tuple, verbose: bool, debug: bool) -> None:
    ...
    async def _validate() -> None:
        ...
        if template.mode == MODE_SINGLE:
            if not secret:
                click.echo(f"{FAILURE_INDICATOR} Template '{template_name}' is in single mode. Please provide a secret.")
            ...
            if var_args:
                click.echo(f"{FAILURE_INDICATOR} Template '{template_name}' is in single mode. Variables are not supported.")
                click.echo(f"Usage: archer validate {template_name} <secret>")
            ...
            if secret:
                click.echo(f"{FAILURE_INDICATOR} Template '{template_name}' is in multipart mode. Please use --var arguments.")
            ...
            if not var_args:
                click.echo(f"{FAILURE_INDICATOR} Template '{template_name}' requires variables.")
